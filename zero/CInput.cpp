...
//The Frame function for the CInput will read the current state of the devices into state buffers we setup.After the state of each device is read it then processes the changes.
...
//I have added a function to the CInput called IsEscapePressed.This shows how to utilize the keyboard to check if specific keys are currently being pressed.You can write other functions to check for any other keys that are of interest to your application.
